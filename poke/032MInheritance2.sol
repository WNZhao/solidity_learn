...
contract Composite is Child1, Child2 {
    // 重写 foo 和 bar 函数，解决菱形继承问题
    function foo() public override(Child1, Base) {
        Child1.foo(); // 调用 Child1 的 foo 函数
    }

    function bar() public override(Child2, Base) {
        Child2.bar(); // 调用 Child2 的 bar 函数
    }
